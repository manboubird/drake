...
(defn- expand-outputs
  ...
  (let [step-set (into #{} step-list)
        ...
        [existing-temp-outputs empty-temp-outputs] (split-with file-exists temp-outputs)
...
(defn- should-build?
  ...
  (let [{:keys [real-inputs real-outputs temp-inputs temp-outputs opts]} (branch-adjust-step step false)
        inputs (into real-inputs (filter file-exists temp-inputs))
        empty-inputs (filter (comp not file-exists) inputs)
    ...
    (if (and (not (empty? empty-inputs)) (or fail-on-empty (not triggered)))
      ...
      (cond
       ...
       (and (not triggered)
            (some (comp not file-exists) outputs)) "missing output"
       ;; timecheck disabled
